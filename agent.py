...
from database.db_queries import (
    ...
    is_lead_already_exists,
    log_chat_transaction,
    create_user_lead_form,
)
...
async def agent_entrypoint(ctx: JobContext):
    ...
    conversation_id = int(
        conversation_id_raw.split("-")[0]
        if "-" in conversation_id_raw
    ...
    try:
        await send_text_message(ctx.room, "message", "Checking credits...")
        ...
        if not credit_check.get("has_credits"):
            ...
            await send_text_message(
                ctx.room, "credit-error", "Insufficient credits to start conversation"
            )
        ...
        await send_text_message(ctx.room, "message", "Initilizing conversation...")
        ...
        await send_text_message(
            ctx.room, "loading", "Initilizing...", {"loading": False}
        )
        await send_text_message(
            ctx.room,
            "credit-error",
    ...
    if is_embed_shared_chatbot:
        ...
        if len(lead_form.__getitem__("chatBotLeadInputField")) > 0:
            is_lead_already_exists_result = await is_lead_already_exists(
                knowledgebase_id,
                lead_form.__getitem__("id"),
                user_session_id,
                conversation_id,
            )
    ...
    async def handle_lead_form_response(user_response: str) -> bool:
        ...
        if not is_lead_form_active or current_lead_form_field_index >= len(
            lead_form_fields
        ):
            return False
        ...
        lead_form_responses.append(
            {"lable": current_field["label"], "value": user_response}
        )
        ...
        if current_lead_form_field_index < len(lead_form_fields):
            ...
            try:
                ...
                if success:
                    await session.say(
                        "Thanks for filling the form, you can now proceed with the conversation"
                    )
                    ...
                    await session.say(
                        "I apologize, but there was an issue saving your information. Let's continue with our conversation."
                    )
    ...
    session = AgentSession(
        llm=groq.LLM(
            model=metadata.get("llmName") or "openai/gpt-oss-20b",
            temperature=0.8,
    ...
    background_audio = BackgroundAudioPlayer(
        ambient_sound=AudioConfig(BuiltinAudioClip.OFFICE_AMBIENCE, volume=1.35),
        thinking_sound=[
    ...
    async def handle_lk_chat(text_reader):
        ...
        if not credit_check.get("has_credits"):
            ...
            await send_text_message(
                ctx.room,
                "credit-error",
                "Insufficient credits to continue conversation",
            )
    ...
    if is_embed_shared_chatbot:
        if is_lead_form_active:
            await send_text_message(ctx.room, "message", "Starting lead form...")
    ...
    def on_user_input_transcribed(user_input: str):
        print(f"User input transcription: {user_input}")
